...
function execGit(command) {
  try {
    ...
    // Return empty string if git is not available (e.g., in Vercel build environment)
    return ''
  }
}
...
function main() {
  try {
    ...
    console.log('⚠️  Continuing build without releases data (git not available)')
    // Create empty releases file so build can continue
    const outputPath = path.join(process.cwd(), 'public', 'data', 'releases.json')
    const emptyData = { releases: [], stats: { total: 0, byType: { major: 0, minor: 0, patch: 0 } } }
    writeReleasesFile(emptyData, outputPath)
  }
}
