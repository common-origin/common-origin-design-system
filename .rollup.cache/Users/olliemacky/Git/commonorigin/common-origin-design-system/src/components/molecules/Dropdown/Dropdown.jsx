...
export var Dropdown = function (_a) {
    ...
    return (<DropdownContainer ref={dropdownRef} className={className}>
      ...
      <DropdownMenu $isOpen={isOpen} role="listbox" aria-labelledby={dropdownId}>{options.map(function (option, index) { return (<DropdownOption key={option.id} $isSelected={option.id === value} $isFocused={index === focusedIndex} onClick={function () { return handleOptionClick(option.id); }} role="option" aria-selected={option.id === value}>
            {option.label}
          </DropdownOption>); })}
      </DropdownMenu>
    </DropdownContainer>);
