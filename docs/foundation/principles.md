...
Every colour, spacing, radius, shadow, font, z-index, and duration comes from a design token. Components use semantic tokens, or component tokens where a component departs from the semantic tier or a family shares a decision ([0014](decisions/0014-token-tiers.md)); base tokens exist to build semantic tokens. If the right token doesn't exist, add one — don't hard-code.
